<template>
  ...
  <div>
    <div class="flex justify-between items-center py-2 border-b">
      <!-- Zone qui toggle l'accordéon -->
      <div class="flex items-center session-hover cursor-pointer" @click="toggleAccordion">
        <span :class="[isOpen ? 'rotate-0' : 'rotate-270']"
              class="material-symbols-outlined session-arrow transition-transform duration-300 mr-2">
          expand_more
        </span>
    ...
    <div v-if="showPopup" class="fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center z-50">
      <div class="bg-white p-6 rounded-lg shadow-md w-[90%] max-w-md">
        ...
        <form class="grid grid-cols-2 gap-4 text-sm text-gray-700" @click.stop>
          <!-- Âge -->
          <div>
        ...
        <div class="flex justify-end space-x-2 mt-6">
          ...
          <button @click.stop="editMode ? submitEditConstraint() : submitNewConstraint()"
    ...
    <div v-if="isOpen" class="mt-2">
      ...
      <div v-for="(constraint, index) in sessionConstraints" :key="constraint.id" class="mb-2">
        ...
        <div class="flex justify-between items-center cursor-pointer py-1 session-sub-hover"
             @click="toggleSession(index)">
          <div class="flex items-center">
            <span :class="[openSessions.includes(index) ? 'rotate-0' : 'rotate-270']"
          ...
          <div v-if="!localIsMobile && userRole === 'ROLE_ADMIN'" class="flex space-x-2 items-center">
            <span
                class="material-symbols-outlined cursor-pointer small-icon hover:text-green-600"
                  title="Éditer cette contrainte"
                  @click.stop="editConstraint(constraint)">
              edit
            </span>
            <span class="material-symbols-outlined cursor-pointer small-icon hover:text-red-600"
        ...
        <!-- Détails de la contrainte -->
        <div v-if="openSessions.includes(index)" class="ml-6 mt-2">
          <table class="w-full text-sm text-gray-600">
            <tbody>
            <tr><td class="text-left">Âge</td><td class="text-center">{{ constraint.infAge }} - {{ constraint.supAge }}</td></tr>
            <tr><td class="text-left">Effectif</td><td class="text-center">{{ constraint.infGroup }} - {{ constraint.supGroup }}</td></tr>
            <tr><td class="text-left">Durée</td><td class="text-center">{{ (constraint.duration / 60).toFixed(1) }}h</td></tr>
            <tr><td class="text-left">Diff. niveau</td><td class="text-center">{{ constraint.levelDiff }}</td></tr>
            <tr><td class="text-left">Diff. âge</td><td class="text-center">{{ constraint.ageDiff }}</td></tr>
            <tr><td class="text-left">Niveau</td><td class="text-center">{{ constraint.infLevel }} - {{ constraint.supLevel }}</td></tr>
            </tbody>
          </table>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async submitNewConstraint() {
      try {
        await this.createSessionConstraint(this.newConstraint);
